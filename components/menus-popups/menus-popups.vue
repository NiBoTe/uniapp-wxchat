<template>
	<view class="mask" :class="!show?'':'mask-show'" :style="{backgroundColor:show?maskBg:'rgba(0,0,0,0)'}"
		@tap="tapMask">
		<view class="popups" v-if="type=== 'default'" :class="[theme]"
		...
		<view v-else class="popups custom" :class="[theme]"
			:style="{top: popupsTop ,left: popupsLeft,flexDirection:direction}">
			<text :class="dynPlace" :style="{width:'0px',height:'0px'}" v-if="triangle"></text>
			<view class="custom-item u-flex" v-for="(item,index) in popData" :key="index" @tap.stop="tapItem(item)">
				<view class="custom-text">{{item.title}}</view>
				<view class="line" v-if="index !== popData.length -1 "></view>
			</view>
		</view>
	</view>
</template>
