				...
				<view class="item" v-for="(item, index) in list" :key="index" @click="detailTap(item)">
					<view class="item-main u-flex">
						...
						<view class="right">
							...
							<view class="right-subtitle">订单编号：{{item.id}}</view>
							<view class="right-subtitle">订单时间：{{item.createdTime}}</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			detailTap(item){
				uni.navigateTo({
					url: `/pages/set/order/paintingEvaluationDetail?orderId=${item.id}`
				})
			}
		}
	}
